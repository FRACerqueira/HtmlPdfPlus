...
namespace ConsoleHtmlToPdfPlus.OnlyAtServerRequestConfig
{
    public class Program
    {
        ...
        {
            Console.WriteLine("Example of HTML console for PDF Plus using only server with all settings on the Request from client");
            Console.WriteLine("===================================================================================================");
            ...
            //create a request with request configuration and without compression to the server 
            //when run in the same context, not Compress is fast because it is not required to transfer data over the network
            var request = RequestHtmlPdf.Create(HtmlSample(), (cfg) =>
            {
                cfg.Margins(10)
                ...
                .ConfigureServices((hostContext, services) =>
                {
                    services.AddHtmlPdfService((cfg) =>
                    {
                        ...
                        cfg.DisableFeatures(DisableOptionsHtmlToPdf.DisableCompress)
                           .AcquireTimeout(1000)// Timeout to acquire Page
                           .AcquireWaitTime(20) // Idle time when page acquisition fails
                           .PagesBuffer(2)      // Max buffer page
